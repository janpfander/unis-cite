...
combined_data  <- combined_data  %>%
  mutate(
    programme_cle = case_when(
      ...
      str_detect(programme_1, regex("Cinéma\\s*&\\s*Citoyenneté", ignore_case = TRUE)) ~ "Cinéma & Citoyenneté",
      ...
      str_detect(programme_1, regex("Santé Mentale", ignore_case = TRUE)) ~ "ASM",
...
To make a binary variable, we include all the programs that contain "Ciné".

```{r}
combined_data <- combined_data %>%
    mutate(
    programme_cine_binary= case_when(
      str_detect(programme_1, "Ciné") ~ 1,
      !is.na(programme_1) ~ 0,
      TRUE ~ NA_real_
    ),
...
combined_data <- combined_data %>%
  mutate(
    ecovolonterre_binary = case_when(
      str_detect(programme_1, "Ecovolonterre") ~ 1,
      !is.na(programme_1) ~ 0,
      TRUE ~ NA_real_
    ),
    ecovolonterre_binary = factor(
      ecovolonterre_binary,
      levels = c(0, 1),
