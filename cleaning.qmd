...
combined_data <- combined_data |> 
  mutate(
    ...
    rupture_valence = case_when(
      ...
      TRUE ~ NA_character_),
    rupture_negative = case_when(
      is.na(motif_rupture) ~ 0,
      rupture_valence == "negative" ~ 1,
      TRUE ~ 0
    ),
    rupture_negative = factor(
      rupture_negative,
      levels = c(0, 1),
...
There is one case for which matching was not possible. Remove this case. 
...
Also, make a factor variable.

```{r}
combined_data <- combined_data |> 
  mutate(type_volontaire = factor(type_volontaire))
```
...
combined_data <- combined_data %>%
  mutate(
    ...
    confiance_avenir_personnel = factor(
...
## Comparaison utlie autres

```{r}
combined_data <- combined_data %>%
  mutate(
    comparaison_utilite_autres = factor(
      comparaison_utilite_autres,
      levels = c(
        "Moins utile à vous qu'aux autres",
        "Autant utile à vous qu'aux autres",
        "Plus utile à vous qu'aux autres"
      )
    )
  )
```
...
combined_data <- combined_data |>
  mutate(
    niveau_etudes = case_when(
      niveau_etudes == "Infra-bac" ~ "infra_bac",
      niveau_etudes == "Bac à Bac + 2" ~ "bac_bac2",
      niveau_etudes == "Bac + 3 et plus" ~ "bac3_plus",
      TRUE ~ NA_character_
    ),
    niveau_etudes = factor(
      niveau_etudes,
      levels = c("infra_bac", "bac_bac2", "bac3_plus")
    )
  )
```
...
combined_data  <- combined_data  %>%
  mutate(
    ...
    # make it a factor in the desired order
    programme_cle = factor(
      programme_cle,
      levels = c(
        "Solidarité Aidants",
        "Cinéma & Citoyenneté",
...
The shape files only cover metropolitan France. Some sites are in La Réunion; we exclude them here (and count how many volunteers this concerns). Note that some of these site names ("Saint-Denis", "Saint-Pierre", "Saint-Benoît") also exist as communes in metropolitan France---we verified via the `region` variable that these sites are indeed in La Réunion.

```{r}
reunion_sites <- c("La Réunion", "Le Port", "Saint-Benoît", "Saint-Denis", "Saint-Pierre")

# volunteers in La Réunion (excluded from the metropolitan map)
n_jeunes |>
  filter(site %in% reunion_sites) |>
  summarise(n = sum(n))

n_jeunes_metro <- n_jeunes |>
  filter(!site %in% reunion_sites)
```

Some sites carry the name of a département rather than a commune (e.g. "Val-d'Oise"). We match those directly at the département level. (Paris matches both a commune and a département; the direct match treats it as a département, which is equivalent.)

```{r}
departement_matches <- n_jeunes_metro |>
  inner_join(departements |> st_drop_geometry() |> select(NOM, INSEE_DEP),
             by = join_by(site == NOM))
```

All remaining sites are matched to communes. Several different communes share the exact same name (e.g. there is a "Valence" in the Drôme, in the Charente and in the Tarn-et-Garonne). Simply joining on commune names would therefore count the same volunteers in several départements at once. For the site names concerned, we assign the correct département by hand; the assignments were verified against the `region` variable in the data.

```{r}
# hand-checked département assignments for site names that match several communes
ambiguous_sites <- tribble(
  ~site, ~INSEE_DEP,
  "Allonnes", "72",       # Sarthe (region PDL in the data)
  "Flers", "61",          # Orne (NOR)
  "La Rochelle", "17",    # Charente-Maritime (NVA)
  "Rochefort", "17",      # Charente-Maritime (NVA)
  "Saint-Nazaire", "44",  # Loire-Atlantique (PDL)
  "Valence", "26"         # Drôme (AURA)
)

commune_matches <- n_jeunes_metro |>
  anti_join(departement_matches, by = c("site", "promo")) |>
  inner_join(communes |> st_drop_geometry() |> select(NOM, INSEE_DEP) |> distinct(),
             by = join_by(site == NOM),
             # same-named communes create multiple matches; resolved right below
             relationship = "many-to-many") |>
  left_join(ambiguous_sites, by = "site", suffix = c("", "_correct")) |>
  filter(is.na(INSEE_DEP_correct) | INSEE_DEP == INSEE_DEP_correct) |>
  select(-INSEE_DEP_correct)

# check: no site should be matched to more than one département anymore
commune_matches |>
  count(site, promo) |>
  filter(n > 1)
```

Sites that match neither a département nor a commune are dropped; this only concerns "Manche Nord" (19 volunteers).

```{r}
n_jeunes_metro |>
  anti_join(departement_matches, by = c("site", "promo")) |>
  anti_join(commune_matches, by = c("site", "promo")) |>
  group_by(site) |>
  summarise(n = sum(n))
```

We then aggregate everything to the département level---**adding up** département-level and commune-level matches (an earlier version of this code used `coalesce()` here, which silently discarded commune-level counts in départements that were also matched directly by a site name). We complete the grid so that every département appears for every promo, with NA where there were no volunteers (so that they show as white on the maps).

```{r}
promos <- c("20-21", "21-22", "22-23", "23-24")

department_counts <- bind_rows(departement_matches, commune_matches) |>
  group_by(INSEE_DEP, promo) |>
  summarise(n = sum(n), .groups = "drop")

department_level_long <- crossing(INSEE_DEP = departements$INSEE_DEP, promo = promos) |>
  left_join(department_counts, by = c("INSEE_DEP", "promo"))
```
